...
SNP.rm.duplicated <- function(x, by="chr:pos", na.keep=TRUE) {
  ...
  if(a$swap_reference > 0) 
    warning(a$swap_reference, " reference allele inversions were performed to remove duplicated SNP")
  if(a$flip_strand > 0)
    warning(a$flip_strand, " allele strand flips were performed to remove duplicated SNP")
  if(a$NAs > 0) 
