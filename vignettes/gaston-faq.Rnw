...
\subsection{Can I retrieve the SNP effects after fitting a linear mixed model?}

Yes, although it usually does not give very good estimates. Hereafter an example code to compute Best 
...
number of SNPs, the SNP effects would be retrieved well. Try to
run the previous code with a $1000\times 5$ random SNP matrix 
built as follows:
...
A few matrix identities leed to an equivalent (and simpler) code
